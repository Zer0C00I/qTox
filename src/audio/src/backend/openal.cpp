...
void applyGain(int16_t* buffer, uint32_t bufferSize, qreal gainFactor)
{
    for (quint32 i = 0; i < bufferSize; ++i) {
        ...
        buffer[i] = static_cast<int16_t>(
            qBound<int>(std::numeric_limits<int16_t>::min(), qRound(buffer[i] * gainFactor),
    ...
    , audioThread{new QThread}
{
    ...
    connect(this, &OpenAL::startActive, this, [this](qreal time) { voiceTimer.start(static_cast<int>(time)); });
...
bool OpenAL::initInput(const QString& deviceName, uint32_t channels)
{
    ...
    inputChannels = static_cast<int>(channels);
...
bool OpenAL::initOutput(const QString& deviceName)
{
    ...
    // init master volume
    alListenerf(AL_GAIN, static_cast<ALfloat>(settings.getOutVolume()) * 0.01f);
    checkAlError();
...
void OpenAL::playMono16Sound(AlSink& sink, const IAudioSink::Sound& sound)
{
    ...
    alBufferData(bufid, AL_FORMAT_MONO16, data.constData(), static_cast<ALsizei>(data.size()), AUDIO_SAMPLE_RATE);
    alSourcei(sourceId, AL_BUFFER, static_cast<ALint>(bufid));
...
void OpenAL::doInput()
{
    ...
    for (auto* source : sources) {
        emit source->frameAvailable(inputBuffer, AUDIO_FRAME_SAMPLE_COUNT_PER_CHANNEL,
                                    static_cast<uint8_t>(inputChannels), AUDIO_SAMPLE_RATE);
    }
}
...
QStringList OpenAL::outDeviceNames()
{
    ...
    if (pDeviceList != nullptr) {
        while (*pDeviceList != 0) {
            auto len = static_cast<qsizetype>(strlen(pDeviceList));
...
void OpenAL::cleanupBuffers(uint sourceId)
{
    ...
    assert(processed >= 0);
    bufids.resize(static_cast<size_t>(processed));
